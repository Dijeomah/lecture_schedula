...
const { checkValidation, toSha512, formatDate, randGen, providusDateFormat} = require('../utils/common.utils');
...
class NotificationController {
    fromSquadco = async (req, res, next) => {
        ...
        if (accountProvider == null) {
            await providerNotification.update({appstatus: "failed"})
    ...
    fromGlobus = async (req, res, next) => {
        ...
        if ((req.headers['clientid'] ?? req.headers['ClientID']) == provider.credentials.cliendID) {
            let { TransID: reference, VirtualAccount: account, Amount: amount, TransactionDate: date, NubanAccountName: originator, narration: description } = req.body;
            originator = req.body.NubanAccount + "|" + originator;

            await providerNotification.update({accountNumber: account})
    ...
    fromProvidus = async (req, res, next) => {
        checkValidation(req);
        logger.info("Callback from Providus:");
        logger.info(req.body);
        const provider = await Provider.findOne({ where: { code: "providus" } });
        ...
        if ((req.headers['client-id'] ?? req.headers['Client-Id']) === provider.credentials.cliendID) {
            let {
                reference: reference,
                accountNumber: account,
                amount: amount,
                dateTime: date,
                narration: originator,
                remark: description
            } = req.body;
            originator = req.body.accountNumber + "|" + originator;

            await providerNotification.update({ accountNumber: account })

            return this.__handleNotification(res, provider, reference, account, parseFloat(amount), providusDateFormat(date), originator, description, req.body, providerNotification);
        } else {
        ...
        try {
            ...
            if (!account) {
                ...
                if (account.Channel.webhookUrl != null) {
                    await notificationService.sendWebhook(account, notification, providerNotification);
                }
            }
        } catch (e) {
