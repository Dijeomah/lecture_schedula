...
class VirtualAccountController {
    getAllAccount = async (req, res, next) => {
        ...
        return respondSuccess(res, "Virtual account list retrieved", {
            accounts,
            currentPage: page,
            perPage: count,
            total: totalItems,
    ...
    getAllAccountTrans = async (req, res, next) => {
        ...
        const totalPages = Math.ceil(totalItems / limit);
        return respondSuccess(res, "Transaction list retrieved", {
            transactions,
            currentPage: page,
            perPage: count,
            total: totalItems,
    ...
    getAccountTransactions = async (req, res, next) => {
        ...
        const transactions = await TransactionNotification.findAll({
            ...
            where: {accountNumber: account, ChannelId: channel.id},
            attributes: {
                exclude: ['providerNotification', 'channelResponse', 'ip', 'feeCharge', 'deletedAt', 'ChannelId']
            },
            include: [
                {
                    model: VirtualAccount,
    ...
    addPosVirtualAccount = async (req, res, next) => {
        ...
        return respondSuccess(res, "Virtual account created successfully", {
    ...
    addVirtualAccountManually = async (req, res, next) => {
        ...
        const {provider: providerCode, channel: channelUUID, accountName, accountNumber, bvn, phoneNumber} =
            req.body;
        ...
        try {
            ...
            logger.info(`Data used ${bvnAlt} ${phoneNumberAlt}`
            )
            ...
            return respondSuccess(res, "Virtual account created successfully", {
                ...
                accountName: accountNew.accountName,
                bvn: accountNew.bvn,
                phoneNumber: accountNew.phoneNumber,
                Provider: {name: provider.name}
            });
        } catch (e) {
            logger.info(e)
    ...
    addVirtualAccount = async (req, res, next) => {
        ...
        try {
            ...
            const response = await
                switchProviderCall(provider, channel, phoneNumber, accountName, bvn, phoneNumber, settlementAccount.accountNumber, req);
            ...
            return respondSuccess(res, "Virtual account created successfully", {
                    ...
                    bvn: account.bvn,
                    phoneNumber: account.phoneNumber,
                    Provider: {name: provider.name.replace(" Agency", "")}
                }
            );
    ...
    registerVirtualAccount = async (req, res, next) => {
        ...
        try {
            ...
            if (account) {
                account.update({
                        accountNumber, accountName, settlementAccount, tid
                    }
                );
