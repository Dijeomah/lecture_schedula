...
class ChannelController {
    getAllChannels = async (req, res, next) => {
        ...
        return respondSuccess(res, "channel list retrieved", {
            channels,
            currentPage: page,
            perPage: count,
            total: totalItems,
    ...
    addChannel = async (req, res, next) => {
        checkValidation(req);
        let {name, email, channelType, webhookUrl, bearer, feeCharge, feeCap, prefix} = req.body;
        ...
        try {
            const channel = await Channel.create({
                name, email, channelType, webhookUrl, bearer, feeCharge, feeCap, prefix
            });
            ...
            return respondSuccess(res, "channel created successfully", {
                uuid: channel.uuid,
                apiKey: token,
